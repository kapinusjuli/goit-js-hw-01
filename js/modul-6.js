...
// // const array = users.map(user => user.name);

// // const getUserNames = array => array.map(arr => arr.name);
...
// // Write code under this line
// const getUsersWithGender = (array, _gender) => array.filter(({gender}) => gender === _gender).map(({name}) => name);
...
.filter((value, index, skillArray) => skillArray.indexOf(value) === index).sort();
